...
images_flat_phase_5 = images_phase_5.reshape(images_phase_5.shape[0], -1)

# Data Splitting 
X_train, X_val, y_train, y_val = train_test_split(images_flat_phase_5, labels_phase_5, test_size=0.3, random_state=25, stratify=labels_phase_5)

# Model Building 
lr5 = LogisticRegression()
lr5.fit(X_train, y_train)
y_pred = lr5.predict(X_val)
...
sns.heatmap(conf_matrix, annot=True, fmt="d", cmap="cividis", xticklabels=['Benign', 'Malignant'], yticklabels=['Benign', 'Malignant'])
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title('Phase 5 Confusion Matrix (Logistic Regression)')
plt.show()
...
y_test_pred = lr5.predict(test_images_flat)
test_accuracy_1 = accuracy_score(test_labels, y_test_pred)
test_classification_report = classification_report(test_labels, y_test_pred, target_names=['Benign', 'Malignant'])
print("Logistic Regression - Phase 5 (Test Set)")
print("----------------------------")
...
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title('Phase 5 Test Set Confusion Matrix (Logistic Regression)')
plt.show()