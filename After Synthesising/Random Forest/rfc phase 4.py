...
# Model Building
rfc4 = RandomForestClassifier(random_state=25)
rfc4.fit(X_train, y_train)
y_val_pred = rfc4.predict(X_val)
...
plt.title('Phase 4 Confusion Matrix (RFC)')
plt.show()
...
y_test_pred = rfc4.predict(test_images_flat)
test_accuracy = accuracy_score(test_labels, y_test_pred)
test_classification_report_1 = classification_report(test_labels, y_test_pred, target_names=['Benign', 'Malignant'])
print("RFC - Phase 4 (Test Set)")
print("----------------------------")
...
plt.figure(figsize=(8, 6))
sns.heatmap(test_conf_matrix, annot=True, fmt="d", cmap="viridis", xticklabels=['Benign', 'Malignant'], yticklabels=['Benign', 'Malignant'])
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title('Phase 4 Test Set Confusion Matrix (RFC)')
plt.show()