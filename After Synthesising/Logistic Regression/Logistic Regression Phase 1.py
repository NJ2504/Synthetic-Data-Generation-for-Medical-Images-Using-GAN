...
# Loading Phase-1 Synthetic Data 
synthetic_images, synthetic_labels = load_images(synthetic_dir, 1)
synthetic_images_phase_1 = synthetic_images[:520]
synthetic_labels_phase_1 = [1] * 520
...
y_pred_1 = lr1.predict(X_val_1)
print("Accuracy:", accuracy_score(y_val_1, y_pred_1))
print("Classification report: ",{classification_report(y_val_1, y_pred_1)})
print("Confusion matrix: ",{confusion_matrix(y_val_1, y_pred_1)})

# Model evaluation
conf_matrix_1 = confusion_matrix(y_val_1, y_pred_1)
plt.figure(figsize=(8, 6))
sns.heatmap(conf_matrix_1, annot=True, fmt="d", cmap="viridis", xticklabels=['Benign', 'Malignant'], yticklabels=['Benign', 'Malignant'])
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title('Phase 1 Confusion Matrix (Logistic Regression)')
plt.show()
...
y_test_pred_1 = lr1.predict(test_images_flat)
...
print("Logistic Regression - Phase 1 (Test Set)")
print("----------------------------")
...
plt.figure(figsize=(8, 6))
sns.heatmap(test_conf_matrix_1, annot=True, fmt="d", cmap="viridis", xticklabels=['Benign', 'Malignant'], yticklabels=['Benign', 'Malignant'])
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title('Phase 1 Test Set Confusion Matrix (Logistic Regression)')
plt.show()
