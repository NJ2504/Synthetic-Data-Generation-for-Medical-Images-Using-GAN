...
# Loading Phase-2 Synthetic Data 
synthetic_images, synthetic_labels = load_images(synthetic_dir, 1)
synthetic_images_phase_2 = synthetic_images[:1040]
synthetic_labels_phase_2 = [1] * 1040
images_phase_2 = np.array(benign_images + malignant_images + synthetic_images_phase_2)
labels_phase_2 = np.array(benign_labels + malignant_labels + synthetic_labels_phase_2)

# Normalise and Reshaping
images_phase_2 = images_phase_2 / 255.0
images_flat_phase_2 = images_phase_2.reshape(images_phase_2.shape[0], -1)

# Data Splitting 
X_train, X_val, y_train, y_val = train_test_split(images_flat_phase_2, labels_phase_2, test_size=0.3, random_state=25, stratify=labels_phase_2)
...
sns.heatmap(conf_matrix_1, annot=True, fmt="d", cmap="magma", xticklabels=['Benign', 'Malignant'], yticklabels=['Benign', 'Malignant'])
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title('Phase 2 Confusion Matrix (Logistic Regression)')
plt.show()
...
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title('Phase 2 Test Set Confusion Matrix (Logistic Regression)')
plt.show()