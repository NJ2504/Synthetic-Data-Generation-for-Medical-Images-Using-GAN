...
from tensorflow.keras.applications import EfficientNetB5 # type: ignore
...
# Model Building
base_model = EfficientNetB5(weights='imagenet', include_top=False, input_shape=(140, 140, 3))
x = base_model.output
...
print(f"Accuracy (EfficientNetB5): {val_accuracy:.4f}")
val_report = classification_report(y_val, y_val_pred, target_names=['Benign', 'Malignant'])
print("Classification Report (EfficientNetB5):\n", val_report)
...
print(f"Test Accuracy (EfficientNetB5): {test_accuracy:.4f}")
test_report = classification_report(y_test, y_test_pred, target_names=['Benign', 'Malignant'])
print("Test Classification Report (EfficientNetB5):\n", test_report)

test_conf_matrix = confusion_matrix(y_test, y_test_pred)
plt.figure(figsize=(8, 6))
sns.heatmap(test_conf_matrix, annot=True, fmt="d", cmap="inferno", xticklabels=['Benign', 'Malignant'], yticklabels=['Benign', 'Malignant'])
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title('Test Confusion Matrix (EfficientNetB5)')
plt.show()