...
synthetic_images_phase_1 = synthetic_images[:1040]
synthetic_labels_phase_1 = [1] * 1040
...
# Model Building
svc2 = SVC(random_state=25)
svc2.fit(X_train_1, y_train_1)
y_val_pred_1 = svc2.predict(X_val_1)
...
plt.title('Phase 2 Confusion Matrix (SVM)')
plt.show()
...
y_test_pred_1 = svc2.predict(test_images_flat)
...
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title('Phase 2 Test Set Confusion Matrix (SVM)')
plt.show()