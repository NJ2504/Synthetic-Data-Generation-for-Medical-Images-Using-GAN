...
# Model Building
rfc1 = RandomForestClassifier(random_state=25)
rfc1.fit(X_train_1, y_train_1)
y_val_pred_1 = rfc1.predict(X_val_1)
...
plt.title('Phase 1 Confusion Matrix (RFC)')
plt.show()
...
y_test_pred_1 = rfc1.predict(test_images_flat)
...
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title('Phase 1 Test Set Confusion Matrix (RFC)')
plt.show()