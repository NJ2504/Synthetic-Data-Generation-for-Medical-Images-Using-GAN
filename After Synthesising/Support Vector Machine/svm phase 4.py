...
synthetic_images_phase_1 = synthetic_images[:2080]
synthetic_labels_phase_1 = [1] * 2080
...
# Model Building
svc4 = SVC(random_state=25)
svc4.fit(X_train_1, y_train_1)
y_val_pred_1 = svc4.predict(X_val_1)
...
plt.title('Phase 4 Confusion Matrix (SVM)')
plt.show()
...
y_test_pred_1 = svc4.predict(test_images_flat)
...
plt.figure(figsize=(8, 6))
sns.heatmap(test_conf_matrix_1, annot=True, fmt="d", cmap="cividis", xticklabels=['Benign', 'Malignant'], yticklabels=['Benign', 'Malignant'])
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title('Phase 4 Test Set Confusion Matrix (SVM)')
plt.show()