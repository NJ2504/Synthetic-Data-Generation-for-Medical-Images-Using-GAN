...
# Loading Phase-4 Synthetic Data 
synthetic_images, synthetic_labels = load_images(synthetic_dir, 1)
synthetic_images_phase_4 = synthetic_images[:2080]
synthetic_labels_phase_4 = [1] * 2080
images_phase_4 = np.array(benign_images + malignant_images + synthetic_images_phase_4)
labels_phase_4 = np.array(benign_labels + malignant_labels + synthetic_labels_phase_4)

# Normalise and Reshaping
images_phase_4 = images_phase_4 / 255.0
images_flat_phase_4 = images_phase_4.reshape(images_phase_4.shape[0], -1)

# Data Splitting 
X_train, X_val, y_train, y_val = train_test_split(images_flat_phase_4, labels_phase_4, test_size=0.3, random_state=25, stratify=labels_phase_4)

# Model Building   
lr4 = LogisticRegression()
lr4.fit(X_train, y_train)
y_pred = lr4.predict(X_val)
print("Accuracy:", accuracy_score(y_val, y_pred))
print("Classification report: ",{classification_report(y_val, y_pred)})
print("Confusion matrix: ",{confusion_matrix(y_val, y_pred)})

# Model evaluation
conf_matrix_1 = confusion_matrix(y_val, y_pred)
plt.figure(figsize=(8, 6))
sns.heatmap(conf_matrix_1, annot=True, fmt="d", cmap="plasma", xticklabels=['Benign', 'Malignant'], yticklabels=['Benign', 'Malignant'])
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title('Phase 4 Confusion Matrix (Logistic Regression)')
plt.show()
...
plt.figure(figsize=(8, 6))
sns.heatmap(test_conf_matrix, annot=True, fmt="d", cmap="plasma", xticklabels=['Benign', 'Malignant'], yticklabels=['Benign', 'Malignant'])
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title('Phase 4 Test Set Confusion Matrix (Logistic Regression)')
plt.show()