...
synthetic_images_phase_1 = synthetic_images[:2600]
synthetic_labels_phase_1 = [1] * 2600
...
# Model Building
svc5 = SVC(random_state=25)
svc5.fit(X_train_1, y_train_1)
y_val_pred_1 = svc5.predict(X_val_1)
...
plt.title('Phase 5 Confusion Matrix (SVM)')
plt.show()
...
y_test_pred_1 = svc5.predict(test_images_flat)
...
plt.figure(figsize=(8, 6))
sns.heatmap(test_conf_matrix_1, annot=True, fmt="d", cmap="magma", xticklabels=['Benign', 'Malignant'], yticklabels=['Benign', 'Malignant'])
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title('Phase 5 Test Set Confusion Matrix (SVM)')
plt.show()